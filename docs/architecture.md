...
核心体验页面的职责边界见 `docs/features/product-experience-pages.md`；工作空间信息架构见 `docs/features/project-system.md`；Agent 会话、资源挂载和历史管理模型见 `docs/features/agent-system.md`；数据源接入计划见 `docs/features/data-sources.md`。
...
工作空间标签页包括 Markdown、浏览器、Android、Terminal、数据源查询、数据源结果、预览、会话和草稿。Home、设置、账号、订阅等属于全局标签页，不随工作空间切换消失。Terminal 跟随工作空间归属：本地工作空间打开本机 shell，远程工作空间打开远端 shell，并接受权限确认、审计和生命周期管理。数据源查询 Tab 跟随当前工作空间保存查询状态和结果快照，但数据库凭证只进入本机加密存储。
...
│ 会话/文件/浏览 │ 浏览器/文档/Android/预览   │ 当前对话 / 待确认 / 资源    │
└──────────────┴──────────────────────────┴────────────────────────────┘
```

右侧 Agent Panel 只承载当前协作：

- 承载已挂载资源横列、消息流、待确认操作、输入框和底部 Agent/模型/推理模式选择。
- 会话列表、未归档会话和已归档历史统一放到左侧 Activity Bar 的“会话”视图。
...
- 输入框 `@` 用于挂资源，包括项目文件、草稿、打开的文档 Tab、浏览器 Tab、Android/设备 Tab、数据源、数据源查询结果、任务产物和后续云资源。
...
│       ├── claude-code-backend.ts  # 本机 Claude Code CLI 子进程
│       └── backend-factory.ts      # 创建本机 Claude Code 后端
├── agent-core/           # ✅ 本仓库内置 Agent 最小内核
│   ├── runtime/agent-runtime.ts
│   ├── runtime/scope.ts
│   ├── backends/local-claude-code-backend.ts
│   └── tools/tool-host.ts
├── mcp/                  # ✅ MCP 工具系统
...
│   ├── data-source-service.ts # 连接配置、查询、记录读取
│   ├── data-source-ipc.ts     # IPC 白名单
│   ├── credential-store.ts    # 数据源凭证加密存储
│   ├── audit-log.ts           # 查询审计日志
│   └── adapters/              # ES/PostgreSQL/MySQL 等适配器
├── settings/             # ✅ 设置持久化
...
└── data-source/          # 数据源系统（规划：只读查询 + Agent 挂载）
```
...
├── components/
│   ├── activity-bar/         # 左侧图标栏
│   ├── sidebar/              # 侧栏面板（工作空间区、文件树、搜索、同步、数据源）
│   ├── workbench/            # 主工作区
...
│   │   ├── DataSourceQueryTab.tsx # 📋 数据源查询 Tab（规划）
...
📋 未开始的组件目录：`im-panel/`、`editor/`（独立编辑器目录，当前编辑器实现在 `workbench/`）、`data-sources/`。
...
#### 2. Agent 系统（本机 Claude Code）

M9 开源底座先收敛为本机 Claude Code Agent：用户自己安装、登录和配置 Claude Code，DeepInk 负责把浏览器、编辑器和 MCP 工具通过本机进程暴露给 Claude Code。
...
│  ┌────────────┐│
│  │Claude Code ││ ← 本机 CLI 子进程 + stream-json
│  └────────────┘│
└────────────────┘
```

- **本机 CLI 检测**：设置页检测 `/opt/homebrew/bin/claude`、`/usr/local/bin/claude`、用户本地 bin 和 shell PATH，并允许手动填写路径
- **模型服务交给 Claude Code**：DeepInk 第一版不保存模型 API Key，不代理模型服务
- **设置热重载**：切换 Claude Code 路径和预算后重建后端，无需重启
...
└── DataSourceToolModule 📋 — 数据源只读工具（list/search/get_record/run_saved_query）
```

所有工具模块实现统一的 `ToolModule` 接口，通过 `toolHost.registerModule()` 注入，加新工具域无需改 host。

Data Source 工具第一版只读，默认面向 Elasticsearch 搜索和记录读取。Agent 不能静默扫描所有 index；首次读取某个数据源或 Saved Query 需要用户确认，工具响应必须做条数、字节数和 raw 字段限制。详细计划见 `docs/features/data-sources.md`。
...
│   └── history.json          # 同步历史
├── data-source/
│   ├── connections.json      # 数据源非敏感配置（endpoint、默认 index、显示名等）
│   ├── saved-queries.json    # Saved Queries 与字段映射
│   ├── audit-log.jsonl       # 查询审计日志（metadata，不记录完整敏感结果）
│   └── secrets.enc           # 数据源凭证加密引用（或 macOS Keychain 条目）
...
macOS Keychain (DeepInk)      # 认证 Token + WebDAV 密码 + 数据源凭证 + 未来扩展凭据
```
...
9. **数据源最小权限** — 数据源访问只通过主进程，Renderer 不接触明文凭证；Agent 数据源工具第一版只读、需授权、限制结果大小（📋 未开始）
10. **AI 记忆用户可控** — 用户可查看、编辑、删除 AI 的所有记忆（📋 未开始）
