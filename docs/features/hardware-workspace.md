...
> 状态：规划稿
> 最后更新：2026-07-14
> 关联文档：`docs/features/product-milestones.md`、`docs/features/fpc-shape-change-assistant.md`、`docs/features/browser-automation.md`、`docs/features/platform-automation.md`、`docs/features/project-system.md`

## 结论

硬件工作区是 DeepInk 面向 AI 眼镜、PCB、FPC、结构件和小批量生产项目的垂直工作流。
...
DeepInk 在硬件项目里的定位是“硬件生产副驾驶”，不是 EDA 的替代品。
...
当前已实现 Gerber 层类型识别、单层文本预览、基础线段/圆弧 SVG 预览、闭合外形候选识别和基础角色分类；尚未实现完整 Gerber 图形渲染、层叠显示和几何编辑。
...
目标：打开 AI 眼镜目录后，DeepInk 能识别这是硬件项目，并列出关键产物。
...
目标：让 DeepInk 帮用户走到嘉立创报价页，但不自动提交订单。
...
如果硬件能力做成新的顶层 App，而不是工作空间内的能力视图，DeepInk 的信息架构会再次变乱。

如果没有源工程，DeepInk 只能做生产检查和调试辅助，不能假装自己能可靠修改电路板。

下一步最该做 M8.1 和 M8.2：先让当前 AI 眼镜目录被识别、被检查、能生成一份有用的生产风险报告。
