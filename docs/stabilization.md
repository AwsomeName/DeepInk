...
当前进度：S1.1 已开始，库存和优先级见 `docs/ops/stabilization-s1-security-inventory.md`。首包关闭微信公众号 HTML 直注入并启用主 renderer sandbox；S1 尚未完成。下一包必须优先迁移普通设置中的 Agent/Meshy 明文密钥，不得用 CSP 或低风险清理替代。
