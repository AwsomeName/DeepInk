...
当前进度：S2.1 本地候选已完成。自动化 runtime 已拆除 Playwright/ADB 对 Agent 和 MCP 的硬依赖，Editor 与各 MCP 工具模块分别初始化；能力状态统一为 `ready`、`degraded`、`unavailable`、`failed`，并进入设置页和脱敏诊断报告。失败注入测试、`pnpm verify`、`pnpm smoke:standalone` 与严格认证窗口 smoke 均通过。S2 尚未关闭：Data Source、Hardware、CAD、Terminal 和官方集成仍位于 `bootstrapMainProcessServices` 串行启动路径，下一批必须继续拆除这些主服务硬依赖并补启动失败注入。
