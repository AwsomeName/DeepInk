...
`cclink-studio` 现在应被视为 **CCLink 的开源桌面工作台壳**。它可以从源码独立运行，默认路径不应加载登录、订阅、云同步、CCLink/官方消息网络、网络工作区或官方发布链路。

当前不存在额外拆分出的云端或 Agent 独立项目。真实边界如下：
...
- 主进程：`auth`、`subscription`、`sync`、`cclink`、`network`、`私有服务配置`、商业 IPC、商业 terminal adapter。
- Renderer：登录页、订阅页、同步面板、CCLink 面板、网络文件树、网络会话侧栏、相关 stores/utils。
- Shared：`auth/subscription/sync/cclink/network` IPC contract、`legacy-protocol`、`network-*`、`sync-types`。
- 发布：旧制品上传脚本已删除；`scripts/package.sh` 只保留本地打包；`electron-builder.yml` 不再配置默认发布源。
- 依赖：默认开源依赖已移除 官方消息网络、WebDAV、制品上传、QRCode 相关包。
...
| 本地 Terminal | `src/main/terminal/`, `src/main/ipc/terminal-ipc.ts` | 使用中性 `executionError`，不依赖 network error model。 |
...
- `window.deepink.network`
- `@shared/ipc/auth`
- `@shared/ipc/subscription`
- `@shared/ipc/sync`
- `@shared/ipc/cclink`
- `@shared/ipc/network`
- `@shared/legacy-protocol`
- `私有服务配置`
- 旧网络错误模型和错误码
- 旧制品上传脚本和相关环境变量
- 官方消息 SDK 包
...
- `CCLink Studio`：只允许出现在兼容路径、历史测试 fixture、待清理历史文档，或下一轮明确列出的用户可见遗漏中。
- `deepink`：只允许出现在 `window.deepink`、旧 localStorage key、旧路径 fixture、`deepink-agent` 枚举、兼容 env var。
- `network` / `cclink`：只允许作为历史 snapshot 降级、协议文档或未来商业 overlay 接入点，不允许在开源壳默认路径调用已迁走实现。
...
第一问：现在能不能宣布迁移完成？不能。只能说“开源壳默认路径已断开商业模块”。兼容命名仍需分阶段处理。
...
第四问：下一次最容易漏什么？后续新增文档可能继续把订阅、云同步和网络工作区描述成 Studio 内建能力；需要用扫描清单持续压住。
