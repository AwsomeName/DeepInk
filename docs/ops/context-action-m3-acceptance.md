...
> 状态：已完成。H1-H4 于 2026-07-22 全部通过，M3 可以关闭。
...
结果：2026-07-22 真人验收通过。消息引用只生成 Composer 草稿且未自动发送；停止命令只中止目标会话的当前 run，其他 Thread 状态未受影响。
...
结果：2026-07-22 真人验收通过。项目切换时旧菜单立即关闭；项目 B 未出现或执行项目 A 的 Browser、Terminal、Agent 动作；返回项目 A 后原运行现场和登录状态保持正确。
