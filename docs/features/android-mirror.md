...
> 新决策：CCLink Studio 后续不再推进本地 Android 虚拟机 / Google AVD / QEMU 模拟器，也不再把模拟器作为近期产品支柱。Android 方向只保留“用户自有真实手机通过 USB 或 Wi-Fi 连接”的可能性。
...
CCLink Studio 已通过 `WebContentsView` 成功内嵌 Chromium 浏览器，用户操作零延迟，AI 通过 Playwright + CDP 操控。下一步目标是在同一 workbench 区域嵌入 Android 模拟器（Google AVD / QEMU），让用户在窗口内直接看到和操作 Android 应用，AI 通过 ADB 操控。
...
- 拖动 CCLink Studio 窗口时，模拟器窗口靠「监听移动 → 重新 setPosition」追赶，主线程异步派发，**肉眼可见滞后、抖动**
...
#### 1. 在 CCLink Studio 内自研 scrcpy client（推荐）
...
| **推荐方案（scrcpy 自研 client）** | **无需特殊权限** — 视频流与控制均走 ADB，模拟器为 CCLink Studio 启动的可控进程 |
...
- [ ] 本地验证：`scrcpy` 对接 CCLink Studio 使用的 AVD，测量延迟与中文输入
...
- CCLink Studio 自管理 SDK：Electron `userData/android-sdk`，macOS 通常位于 `~/Library/Application Support/<CCLink Studio app name>/android-sdk`。
- CCLink Studio 默认 AVD：`~/.android/avd/CCLink Studio_Phone.avd` 和 `~/.android/avd/CCLink Studio_Phone.ini`。
