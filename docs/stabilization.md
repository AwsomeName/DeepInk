...
当前进度：S1.1-S1.4b 实现和当前工作树本地门禁已完成，库存、失败边界和证据见 `docs/ops/stabilization-s1-security-inventory.md`。S1.1 隔离不可信 HTML 并启用 renderer sandbox；S1.2 将 Agent/Meshy 密钥迁移到 `safeStorage`；S1.3 建立主 renderer CSP 和首批 trusted sender guard；S1.4a 覆盖 Browser、Android 与数据源并撤销高风险旧入口；S1.4b 将 guard 与有界 schema 扩展到所有剩余 renderer IPC，限制 Browser 本地 HTML 到真实工作区资源，收紧更新源和官方集成 registrar，并把 preload 总入口缩减到 179 行。S1 仅等待最新收口提交的全新 detached worktree 复验和远端 CI，不得提前宣称关闭或恢复功能扩张。
