...
当前进度：S2.1 已在基线 `56afb38` 完成，GitHub Actions run `29798156373` 与验收记录提交 `f1b50d1` 的 run `29798447976` 均通过。S2.2 本地候选已将 CAD、Hardware、Data Source、Meshy、Terminal、Official integration 等从 `bootstrapMainProcessServices` 串行硬依赖中拆出，保留降级 IPC、首个失败原因和半初始化清理；137 个测试文件/817 项测试、standalone 与严格认证 smoke 均通过。S2 尚未关闭：`createWindowRuntime` 中 Browser 与 Android 构造仍共享硬启动路径，S2.3 必须继续隔离并补失败注入。
