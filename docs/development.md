...
- `src/shared/ipc/network.ts`
- renderer 登录、订阅、云同步、CCLink、网络工作区 UI/store。
...
- `window.deepink`
- `com.deepink.app`
- Electron legacy `userData`
- `deepink-*` localStorage key
- 历史 fixture 和旧 workspace snapshot 的旧产品文本
...
如果一个文档需要描述官方账号、云函数、官方消息凭证 或付费 entitlement，它大概率不该继续留在 Studio 当前事实源里，而应该转到 `cclink-dev/commercial` 或 `/Users/apple/Desktop/chat-cc`。
