...
> 本文件为 Claude Code 提供项目上下文，每次会话自动加载。
...
│   │   ├── agent/             # Agent 桥接（可插拔后端：Claude Code CLI / HTTP API）
│   │   │   └── backend/       # 后端实现（claude-code、http-api、工厂）
...
- **可插拔后端**：Claude Code CLI + 国内模型 API + HTTP API（OpenAI 兼容）
