...
核心体验页面的职责边界见 `docs/features/product-experience-pages.md`；工作空间信息架构见 `docs/features/project-system.md`。
...
DeepInk 的自动化能力由**两大支柱**构成：**内嵌浏览器**（Web 自动化，AI 操控网页）与**内嵌 Android 模拟器**（移动自动化，AI 操控手机 App，🔧 技术验证中）。两者结构同构——都通过 MCP 工具暴露给 Agent，权限统一管控，操作过程可视化。
...
```
┌──────────────┬──────────────────────────┬────────────────┐
│ Activity/侧栏 │ 主工作区                  │ Agent 面板       │
│              │ 浏览器/文档/Android/预览   │ 对话/工具/确认    │
└──────────────┴──────────────────────────┴────────────────┘
```
...
第一阶段默认采用 **active tab 跟随**：

| Active Tab    | Agent 默认上下文                                  |
| ------------- | ------------------------------------------------- |
| 浏览器        | 当前 URL、页面标题、可见内容、浏览器工具          |
| Markdown 文档 | 当前文件路径、文档内容、编辑器工具                |
| Android       | 当前设备、当前 Activity、截图/UI 树、Android 工具 |
| 文件面板      | 当前选中文件/文件夹、文件系统工具                 |
| 设置页        | 不绑定业务上下文，仅保留通用对话                  |

多 Tab 不默认自动合并上下文，避免 Agent 读取用户未显式授权的内容。后续通过 context chips 显式添加：

```
[当前: 简历.md] [+ 添加上下文]
```

例如用户可以在编辑简历时显式添加“阿里云审核页面”这个浏览器 Tab，让 Agent 执行“读取简历 → 填写网页表单 → 上传文件”的跨上下文任务。
...
#### 8. 内嵌 Android 模拟器（移动自动化支柱）

与浏览器支柱完全对称的第二大自动化能力：在桌面内嵌 Android 模拟器，让 AI 操控手机 App（如微信、各类审核 App）。当前作为新功能已完成全链路技术验证，仍在打磨。

```
Android 模拟器支柱
├── 模拟器全生命周期（emulator-manager.ts）
│   ├── 一键安装：自动下载 SDK + 系统镜像 + 创建默认 AVD
│   ├── 启动/停止 QEMU 模拟器、状态机管理
│   └── 跨平台：macOS/Win/Linux + arm64/x86_64（Apple Silicon 原生虚拟化）
│
├── ADB 桥接（adb-bridge.ts，对标 playwright-bridge.ts）
│   └── 基于 @yume-chan/adb 纯 TS 实现，Node net.Socket 桥接 ADB Server (5037)
│
├── scrcpy 投屏（scrcpy-bridge.ts）
│   ├── 推送 scrcpy-server.jar 到设备
│   ├── 接收 H.264 流 → 转发到渲染进程
│   ├── 渲染进程用 WebCodecs 解码 + WebGL 渲染（AndroidDisplay.tsx）
│   └── 触摸事件回注（含中文输入）
│
└── 15 个 MCP 工具（mcp/modules/android/）
    └── tap/swipe/press_key/type_text/screenshot/dump_ui/device_info/
       list_packages/current_activity/launch_package/go_home/
       install_apk/uninstall_package/push_file/shell
```

**与核心产品的关系**：浏览器支柱解决"AI 操控网页"，Android 支柱解决"AI 操控手机 App"。两者并列，权限统一管控，操作过程可视化。

详见 `docs/features/android-mirror.md`（投屏技术方案）与 `docs/features/cloud-phone.md`（云手机演进方向）。
