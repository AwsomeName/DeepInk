...
网络服务器项目维护仍然是重要商业/官方方向，但不属于 `cclink-studio` OSS 默认能力。网络工作区、网络文件、网络执行、网络 Agent、账号、entitlement 和配对已迁出或封存，不能继续作为当前 OSS MVP 验收项。
...
- archived network model notes
- archived network operations notes
