...
当前进度：S2.1 已在基线 `56afb38` 完成，GitHub Actions run `29798156373` 与验收记录提交 `f1b50d1` 的 run `29798447976` 均通过。S2.2 已在基线 `fd267a2` 完成，将 CAD、Hardware、Data Source、Meshy、Terminal、Official integration 等从 `bootstrapMainProcessServices` 串行硬依赖中拆出，并通过 GitHub Actions run `29799199378`。S2.3 本地候选已隔离 Browser 与 Android 窗口构造，补齐双向失败注入、半初始化清理和 Browser 第一现场保留；138 个测试文件/820 项测试、standalone 与严格认证 smoke 均通过。S2 尚未关闭：S2.3 仍需提交、远端 CI 和全新 detached worktree 复验。设置页刷新只读取真实状态；环境修复后的当前恢复入口是重启 Studio，进程内重试、统一回滚和窗口重建属于 S3。
