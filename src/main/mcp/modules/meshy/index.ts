...
const MESHY_TOOL_DEFINITIONS: ToolDefinition[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        modelType: { type: 'string', enum: ['standard', 'lowpoly'], description: '网格类型，默认 standard' },
        aiModel: { type: 'string', enum: ['meshy-5', 'meshy-6', 'latest'], description: 'Meshy 模型，默认 latest' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        outputDir: { type: 'string', description: '可选保存目录。省略则使用当前工作区 assets/meshy' },
