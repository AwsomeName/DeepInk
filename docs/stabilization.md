...
当前进度：S1.1-S1.3 已完成，库存、失败边界和证据见 `docs/ops/stabilization-s1-security-inventory.md`。S1.1 关闭微信公众号 HTML 直注入并启用主 renderer sandbox；S1.2 将 Agent/Meshy 密钥迁移到 `safeStorage`，公共设置与 renderer 不再读取原文；S1.3 为主 renderer 增加开发/生产分离的 CSP，并为窗口、设置、文件和 Terminal IPC 建立统一 trusted sender guard，设置与文件入口增加运行时 schema。S1 尚未完成，下一包继续覆盖其余高权限 IPC、资源作用域和 preload 暴露面，不得恢复功能扩张。
